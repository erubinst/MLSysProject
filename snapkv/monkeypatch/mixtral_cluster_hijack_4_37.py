...
def mixtral_flash_attn2_forward(
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if hasattr(self, "kv_seq_len"): #[ClusterKV] add kv_seq_len
    ...
    if not self.training and not (dropout_rate > 0):
        # In this case, flash_attn is used
        if attention_mask is not None:
            # Transform attention_mask to the format expected by flash_attn
            ...
            # Convert bool attention_mask to additive attention_mask
            attention_mask = torch.where(attention_mask, 0.0, torch.finfo(query_states.dtype).min)
...
def prepare_inputs_for_generation_mixtral(
    ...
    if past_key_values is None: # [ClusterKV]
